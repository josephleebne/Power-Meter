...
for each sample, for example:
    ...
    [3.14, 0.52, 9.80, 0.95, 1, 18.6, 0.42,
     12.3, 0.87, 50.0, 1.63, 9.55, 1.38, 9.65, 3600]
...
Index mapping:
     ...
     2  AC Voltage RMS (V)
     3  AC Current RMS (A)
     4  Low Current Mode Flag
        (0 = high current mode, 1 = low current mode)
     5  AC Voltage Vpp (V)
     6  AC Current Vpp (A)
     7  Phase Difference (deg)
     8  Power Factor
     9  Frequency (Hz)
    10  DC Power (W)
    11  AC Real Power (W)
    12  AC Reactive Power (VAR)
    13  AC Apparent Power (VA)
    14  RTC time (seconds since midnight)
...
Special values:
    PLACEHOLDER_VALUE (37) -> measurement not yet available
...
# AVR now uses 37 as the default placeholder / UL code.
PLACEHOLDER_VALUE = 37
...
DISPLAY_MEASUREMENTS = [
    ...
    ("AC Frequency (Hz)", 9, "{:.2f}"),
    ("AC Phase Difference (deg)", 7, "{:.2f}"),

    ("AC Real Power (W)", 11, "{:.2f}"),
    ("AC Reactive Power (VAR)", 12, "{:.2f}"),
    ("AC Apparent Power (VA)", 13, "{:.2f}"),

    ("AC Power Factor", 8, "{:.3f}"),
...
class App:
    ...
    def build_top_bar(self):
        ...
        # The RTC label displays index 14 from the AVR measurement array.
    ...
    def handle_serial_item(self, item):
        ...
        # Index 14 is already seconds since midnight from the AVR RTC.
        # No PC time-sync command is sent back to the AVR here.
        rtc_raw = values[14] if len(values) > 14 else None
    ...
    def handle_measurement_array(self, values: list):
        ...
        for name, index, number_format in DISPLAY_MEASUREMENTS:
            ...
            if name == "Low Current Mode":
                mode_text = "LOW" if int(value) == 1 else "HIGH"

                self.value_labels[name].config(
                    text=mode_text,
                    fg=COLOR_ORANGE if int(value) == 1 else COLOR_GREEN,
                )

                self.history[name].append(
                    (offset_s, value, self.sample_seq)
                )
    ...
    def update_plot(self, *_):
        ...
        is_mode_plot = selected == "Low Current Mode"
        ...
        if is_mode_plot:
            ...
            self.ax.set_yticklabels(["HIGH", "LOW"])
        ...
        if visible:
            ...
            if normal:
                ...
                if is_mode_plot:
                    y_values = [1 if value >= 0.5 else 0 for value in y_values]
                self.ax.plot(
                    x_values,
                    y_values,
                    linewidth=1.3,
    ...
    def on_hover(self, event):
        ...
        if selected == "Low Current Mode":
            value_str = "LOW" if int(value) == 1 else "HIGH"
        else:
