...
FUSES = {
    ...
    = 0xE2, // LOW
            ...
            // CKOUT=CLEAR, CKDIV8=CLEAR}
    .high
    = 0xD9, // HIGH {BOOTRST=CLEAR,
            // BOOTSZ=2048W_3800, EESAVE=CLEAR,
            // WDTON=CLEAR, SPIEN=SET, DWEN=CLEAR,
            // RSTDISBL=CLEAR}
    .extended
    = 0xFF, // EXTENDED {BODLEVEL=DISABLED}
};

LOCKBITS = 0xFF; // {LB=NO_LOCK, BLB0=NO_LOCK,
                 // BLB1=NO_LOCK}
...
// Number of samples taken in one window of AC
// measurement
...
// Calibration values
#define AC_VOLTAGE_ERROR_RATIO 1.128
#define AC_VOLTAGE_ERROR_SUM 0.124

#define AC_CURRENT_HIGH_ERROR_RATIO 0.864
#define AC_CURRENT_HIGH_ERROR_SUM -0.375

#define AC_CURRENT_LOW_ERROR_RATIO 0.864
#define AC_CURRENT_LOW_ERROR_SUM -0.375

#define DC_VOLTAGE_ERROR_RATIO 1.545
#define DC_VOLTAGE_ERROR_SUM 0.022

#define DC_CURRENT_ERROR_RATIO 0.965
#define DC_CURRENT_ERROR_SUM 0.012

#define FREQUENCY_ERROR_RATIO 1.0f
#define FREQUENCY_ERROR_SUM 0.0f

#define PHASE_DIFFERENCE_ERROR_RATIO -1.055
#define PHASE_DIFFERENCE_ERROR_SUM -12.736

#define AC_VOLTAGE_PK_PK_ERROR_RATIO 11.439
#define AC_VOLTAGE_PK_PK_ERROR_SUM -0.0375

#define AC_CURRENT_PK_PK_ERROR_RATIO 7.669
#define AC_CURRENT_PK_PK_ERROR_SUM -0.231

// FOR FREQUENCY MEASUREMENT
#define HYSTERESIS 0.5f
#define FREQ_AVG_COUNT \
    10 // Number of cycles to be averaged
// Take the clock speed of 8 million and divide by
// the /64 prescalar
...
// Low Current mode
#define LOW_CURRENT_THRESHOLD \
    1.0f // Low current mode turns on below 1A
uint8_t isLowCurrentMode
    = 0; // 0 means high current mode, 1 means low
         // current mode
#define LOW_CURRENT_SCALING_RATIO \
    0.1f // Current between 0-1A is amplified 10x

// ***************      UNDER LIMIT
// DEFINITIONS/FLAGS      ********************

// flag and definition for checking if AC
// voltage/current dependent measurements are UL
volatile uint8_t
    isUnderLimit; // 1 for is UL, 0 for not UL
#define UNDER_LIMIT_CODE \
    37.0f // Sending arbitrary number when UL
#define CURRENT_UNDER_LIMIT_THRESHOLD \
    0.1 // when AC current is below 0.1, show
        // UNDER_LIMIT_CODE
...
// ************************      TURNS RATIO
// *****************************
float turnsRatio = 26.778f; //26.778

// ******************      LCD FUNCTION PROTOTYPES
// ***********************
uint8_t u8x8_avr_delay(u8x8_t* u8x8, uint8_t msg,
    uint8_t arg_int, void* arg_ptr);
uint8_t u8x8_avr_gpio_and_delay(u8x8_t* u8x8,
...
float MenuSettings[] = {
   ...
   // Measurement 4, BacklightIndex, TurnsRatio
int MenuSettingsEEPROMAddresses[] = {
    MEASUREMENT_1_ADDRESS, MEASUREMENT_2_ADDRESS,
    MEASUREMENT_3_ADDRESS, MEASUREMENT_4_ADDRESS,
    BACKLIGHT_ADDRESS, TURNS_RATIO_ADDRESS
};
...
char* MeasurementName[MEASUREMENTS] = { "DCV (V)",
    "DCC (A)", "ACV (V)", "ACC (A)", "ACVPP (V)",
    "ACCPP (A)", "PD", "PF", "F (Hz)", "DCP (W)",
    "ACRP (W)", "ACRRP (W)", "ACAP (W)" };

int Menu = 0; // 0 = main screen, 1 = measurement
              // select, 2 = settings, 3 = edit
              // turns ratio
int SelectPosition[2]; // 0 = row position for the
                       // main screen, 1 =
                       // secondary position used
                       // for other menus
int Backlight[] = { 0, 64, 128, 192,
    255 }; // Values for OCR0B for Backlight
int BacklightIndexCopy
    = 4; // Copy of Backlight Index
float TurnsRatioCopy
    = 26.788; // Copy of Transformer Turns Ratio
...
uint8_t DS1307_set_time(const rtc_time_t* t)
{
    ...
    if (!TWI_write(dec_to_bcd(t->sec) & 0x7F)) {
        TWI_stop();
        return 0;
    } // clear CH bit
    if (!TWI_write(dec_to_bcd(t->min))) {
        TWI_stop();
        return 0;
    }
    if (!TWI_write(dec_to_bcd(t->hour))) {
        TWI_stop();
        return 0;
    }
    if (!TWI_write(dec_to_bcd(t->dayOfWeek))) {
        TWI_stop();
        return 0;
    }
    if (!TWI_write(dec_to_bcd(t->day))) {
        TWI_stop();
        return 0;
    }
    if (!TWI_write(dec_to_bcd(t->month))) {
        TWI_stop();
        return 0;
    }
    if (!TWI_write(dec_to_bcd(t->year))) {
        TWI_stop();
        return 0;
    }
...
void UART_print_measurements(
    float arr[NO_MEASUREMENTS])
{
    UART_print("[");

    for (uint8_t i = 0; i < NO_MEASUREMENTS;
        i++) {
...
void setup_ADC(void)
{
    // Set voltage reference to AREF and use 10
    // bit results.
    ADMUX = (0 << REFS1) | (0 << REFS0)
        | (0 << ADLAR);

    // Enable interrupts. Set prescalar divisor to
    // 64
...
// Takes in a number from 0 to 4, corresponding to
// an ADC Channel
void ADC_select_channel(uint8_t channel)
{
    ...
    ADMUX &= ~((1 << MUX3) | (1 << MUX2)
        | (1 << MUX1)
        | (1 << MUX0)); // Clear channel
    ADMUX |= channel;
}

uint16_t calculate_midpoint_ADC(
    uint32_t sum, uint16_t count)
{
    ...
    uint16_t mean
        = (uint16_t)((float)sum / (float)count);
    return mean;
}

void handle_frequency_logic(uint16_t reading,
    uint8_t channel, uint16_t voltageMidpoint,
    uint16_t currentMidpoint)
{
    if (channel == AC_VOLTAGE_CHANNEL) {
        int32_t voltageLowThreshold
            = (int32_t)voltageMidpoint
            - HYSTERESIS;
        int32_t voltageHighThreshold
            = (int32_t)voltageMidpoint
            + HYSTERESIS;

        // Detect zero Crossing for voltage
        //(was below the lower bound and is now
        // above the high bound)
        if ((lastACValue <= voltageLowThreshold)
            && (reading > voltageHighThreshold)) {
            // Take the timestamp of this zero
            // crossing and calculate time from
            // previous crossing
            uint16_t currentTime = TCNT1;
            uint16_t duration
                = currentTime - lastVTimestamp;
            lastVTimestamp = currentTime;
            vCrossingTime = currentTime;

            // Update the running total of
            // periods, to be averaged
            totalPeriodAccumulator += duration;
            cycleCounter++;

            if (cycleCounter >= FREQ_AVG_COUNT) {
                periodTicks
                    = totalPeriodAccumulator;
    ...
    if (channel == AC_CURRENT_CHANNEL) {
        int32_t currentLowThreshold
            = (int32_t)currentMidpoint
            - HYSTERESIS;
        int32_t currentHighThreshold
            = (int32_t)currentMidpoint
            + HYSTERESIS;
        ...
        //(was below the lower bound and is now
        // above the high bound)
        if ((lastACCurrentValue
                <= currentLowThreshold)
            && (reading > currentHighThreshold)) {
            // Take the timestamp
            iCrossingTime = TCNT1;
        }
    ...
    */
    ...
    if (!acProcessingBusy) {
        ...
        handle_frequency_logic(reading, channel,
            voltageMidpoint, currentMidpoint);
        ...
        if (channel == AC_VOLTAGE_CHANNEL) {
            ...
            acVoltageSumSq += (uint32_t)reading
                * (uint32_t)reading;
        ...
        if (channel == AC_CURRENT_CHANNEL) {
            ...
            acCurrentSumSq += (uint32_t)reading
                * (uint32_t)reading;
...
// Converts the ADC reading into DC voltage, then
// calculates input DC voltage
float calculate_DC_voltage(uint16_t reading)
{
    ...
    float Vout
        = ADC_counts_to_meas(filteredCounts);

    float vin = (Vout / scalingRatio);
    float vinCorrected
        = vin * DC_VOLTAGE_ERROR_RATIO
        + DC_VOLTAGE_ERROR_SUM;
    return vinCorrected;
}

// Converts the ADC reading into DC current, then
// calculates input DC current
float calculate_DC_current(uint16_t reading)
{
    ...
    float iInCorrected
        = iIn * DC_CURRENT_ERROR_RATIO
        + DC_CURRENT_ERROR_SUM;
    return iInCorrected;
}

float calculate_RMS_ADC(
    uint32_t sum, uint32_t sumSq, uint16_t count)
{
    if (count == 0) { // Prevent 0 division
        return 0.0f;
    }
    ...
    // Prevent negative square roots due to
    // rounding
    if (variance < 0.0f) {
        variance = 0.0f;
    }
...
float calculate_AC_voltage_RMS(
    uint32_t sum, uint32_t sumSq, uint16_t count)
{
    float scalingRatio = (1.4f / 14.14f);

    float RMSCounts
        = calculate_RMS_ADC(sum, sumSq, count);
    float ADCRMSVoltage
        = ADC_counts_to_meas(RMSCounts);

    float vin = (ADCRMSVoltage / scalingRatio)
        * turnsRatio;
    float vinCorrected
        = vin * AC_VOLTAGE_ERROR_RATIO
        + AC_VOLTAGE_ERROR_SUM;
    return vinCorrected;
}

float calculate_AC_current_RMS(
    uint32_t sum, uint32_t sumSq, uint16_t count)
{
    float scalingRatio = 1.0f;
    ...
    float ADCRMSVoltage
        = ADC_counts_to_meas(RMSCounts);
    float currentRMS
        = (ADCRMSVoltage / scalingRatio)
        * currentToVoltageRatio;
    float currentRMSCorrected
        = currentRMS * AC_CURRENT_HIGH_ERROR_RATIO
        + AC_CURRENT_HIGH_ERROR_SUM;

    // Send the under limit (UL) code "37" if
    // current is below 0.1
    if (currentRMSCorrected
        < CURRENT_UNDER_LIMIT_THRESHOLD) {
        currentRMSCorrected
            = UNDER_LIMIT_CODE; // Send "37" to
                                // the GUI/LCD
    }
...
uint8_t check_low_current_mode(float current)
{
    ...
    // If current is below 1A, turn on low current
    // mode, mux select low current circuit
    if (current < LOW_CURRENT_THRESHOLD) {
        PORTD |= (1 << PORTD2);
        isLowCurrentMode = 1;
    }

    // Otherwise, turn off low current mode, mux
    // select the high current circuit
    else {
        PORTD &= ~(1 << PORTD2);
        isLowCurrentMode = 0;
    }
...
float calculate_frequency(uint32_t ticks)
{
    ...
    uint32_t freqConstant
        = T1_TICK_FREQ * (uint32_t)FREQ_AVG_COUNT;
    float freq
        = (float)freqConstant / (float)ticks;
    float freqCorrected
        = freq * FREQUENCY_ERROR_RATIO
        + FREQUENCY_ERROR_SUM;

    return freqCorrected;
}

float calculate_phase_difference(uint16_t vTime,
    uint16_t iTime, uint32_t avgPeriodTicks,
    float current)
{
    ...
    // Get the period of a single cycle from the
    // averaged total
    float singleCycleTicks = (float)avgPeriodTicks
        / (float)FREQ_AVG_COUNT;

    // Calculate time between Vcrossing and
    // Icrossing
    int16_t diff = (int16_t)(iTime - vTime);

    // Convert time difference to degrees
    float phase = ((float)diff / singleCycleTicks)
        * 360.0f;
...
float calculate_power_factor(float phase)
{
    ...
    if (phase == UNDER_LIMIT_CODE) {
        return UNDER_LIMIT_CODE; // Send "37" to
                                 // the GUI/LCD
    }
    // Convert to radians
...
float calculate_AC_real_power(float voltageRMS,
    float currentRMS, float powerFactor)
{
    // Check if the AC current is UL
    ...
    float realPower
        = voltageRMS * currentRMS * powerFactor;
...
float calculate_AC_apparent_power(
    float voltageRMS, float currentRMS)
{
    // Check if the AC current is UL
    if (currentRMS == UNDER_LIMIT_CODE) {
        return UNDER_LIMIT_CODE; // Send "37" to
...
float calculate_AC_reactive_power(
    float voltageRMS, float currentRMS,
    float phaseDifference)
{
    ...
    float phaseRadians
        = phaseDifference * M_PI / 180.0f;

    float reactivePower = voltageRMS * currentRMS
        * sinf(phaseRadians);
...
float calculate_voltage_pk_pk(
    uint16_t rawMax, uint16_t rawMin)
{
    ...
    if (filteredCounts < 0) {
        ...
        filteredCounts
            += (rawPkPk - filteredCounts)
            * PK_PK_SMOOTHING_FACTOR;
    }

    float voltagePkPk
        = ADC_counts_to_meas(filteredCounts);
    float voltagePkPkCorrected = voltagePkPk
            * AC_VOLTAGE_PK_PK_ERROR_RATIO
        + AC_VOLTAGE_PK_PK_ERROR_SUM;

    return voltagePkPkCorrected;
}

float calculate_current_pk_pk(
    uint16_t rawMax, uint16_t rawMin)
{
    // Underflow protection
    if (rawMax < rawMin) {
    ...
    float currentPkPk
        = ADC_counts_to_meas(filteredCounts);
    float currentPkPkCorrected = currentPkPk
            * AC_CURRENT_PK_PK_ERROR_RATIO
        + AC_CURRENT_PK_PK_ERROR_SUM;
...
void init_MCU_GPIO()
{
    // Use D3 to blink LED when a new measurement
    // is taken
    DDRD |= 1 << PORTD3;
...
{
    // If its time to turn the LED off AND the led
    // is on,
    if (isLEDOn
        && timer2Overflows
            >= ledOffTimestamp) { // Every ~100ms
        // Turn off LED
        PORTD &= ~(1 << PORTD3);
        isLEDOn = 0;
    }
...
void trigger_measurement_LED(void)
{
    ...
    ledOffTimestamp
        = timer2Overflows + BLINK_DURATION_TICKS;
    isLEDOn = 1;
}
...
uint8_t process_measurements(float* measurements)
{
    ...
    if (acDataReady) {
        ...
        // Frequency and Phase local timing
        // snapshots
        uint16_t localVoltageMidpoint
            = voltageMidpoint;
        uint16_t localCurrentMidpoint
            = currentMidpoint;
        ...
        // Calculate midpoint
        if (localSamples > 0) {
            localVoltageMidpoint
                = calculate_midpoint_ADC(
                    localVSum, localSamples);
            localCurrentMidpoint
                = calculate_midpoint_ADC(
                    localISum, localSamples);
        } else {
        ...
        measurements[MEAS_DC_VOLTAGE]
            = calculate_DC_voltage(
                rawDCVoltageADC);
        measurements[MEAS_DC_CURRENT]
            = calculate_DC_current(
                rawDCCurrentADC);

        // DC power
        measurements[MEAS_DC_POWER]
            = calculate_DC_power(
                measurements[MEAS_DC_VOLTAGE],
                measurements[MEAS_DC_CURRENT]);
        ...
        measurements[MEAS_AC_VOLTAGE]
            = calculate_AC_voltage_RMS(localVSum,
                localVSumSq, localSamples);

        float currentRMS
            = calculate_AC_current_RMS(localISum,
                localISumSq, localSamples);
        if (isLowCurrentMode) {
            measurements[MEAS_AC_CURRENT]
                = currentRMS
                    * LOW_CURRENT_SCALING_RATIO
                    * AC_CURRENT_LOW_ERROR_RATIO
                + AC_CURRENT_LOW_ERROR_SUM;
        } else {
            measurements[MEAS_AC_CURRENT]
                = currentRMS;
        }
        measurements[MEAS_IS_LOW_CURRENT_MODE]
            = isLowCurrentMode;

        // Calculate frequency, phase difference,
        // and  power factor
        measurements[MEAS_FREQUENCY]
            = calculate_frequency(
                localPeriodTicks);
        measurements[MEAS_PHASE_DIFFERENCE]
            = calculate_phase_difference(
                localVCrossing, localICrossing,
                localPeriodTicks,
                measurements[MEAS_AC_CURRENT]);
        measurements[MEAS_POWER_FACTOR]
            = calculate_power_factor(measurements
                    [MEAS_PHASE_DIFFERENCE]);

        // Calculate AC power (real, apparent,
        // reactive)
        measurements[MEAS_AC_REAL_POWER]
            = calculate_AC_real_power(
                measurements[MEAS_AC_VOLTAGE],
                measurements[MEAS_AC_CURRENT],
                measurements[MEAS_POWER_FACTOR]);
        measurements[MEAS_AC_REACTIVE_POWER]
            = calculate_AC_reactive_power(
                measurements[MEAS_AC_VOLTAGE],
                measurements[MEAS_AC_CURRENT],
                measurements
                    [MEAS_PHASE_DIFFERENCE]);
        measurements[MEAS_AC_APPARENT_POWER]
            = calculate_AC_apparent_power(
                measurements[MEAS_AC_VOLTAGE],
                measurements[MEAS_AC_CURRENT]);

        // Calculate pk to pk voltage and current
        measurements[MEAS_AC_VOLTAGE_PK_PK]
            = calculate_voltage_pk_pk(
                localVMax, localVMin);
        measurements[MEAS_AC_CURRENT_PK_PK]
            = calculate_current_pk_pk(
                localIMax, localIMin);
    ...
    uint8_t arg_int, void* arg_ptr)
{
    ...
    switch (msg) {
    case U8X8_MSG_DELAY_NANO: // delay arg_int * 1
                              // nano second At
                              // 20Mhz, each cycle
                              // is 50ns, the call
                              // itself is slower.
        break;
    case U8X8_MSG_DELAY_100NANO: // delay arg_int
                                 // * 100 nano
                                 // seconds
                                 // Approximate
                                 // best case
...
#define CYCLES_PER_LOOP 4UL
        ...
        cycles = (100UL * arg_int)
            / (P_CPU_NS * CYCLES_PER_LOOP);

        if (cycles > CALL_CYCLES + RETURN_CYCLES
                + CALC_CYCLES)
            break;
        ...
        __asm__ __volatile__("1: sbiw %0,1"
            ...
            : "0"(cycles));
        break;
    case U8X8_MSG_DELAY_10MICRO: // delay arg_int
                                 // * 10 micro
                                 // seconds
        for (int i = 0; i < arg_int; i++)
            _delay_us(10);
        break;
    case U8X8_MSG_DELAY_MILLI: // delay arg_int *
                               // 1 milli second
        for (int i = 0; i < arg_int; i++)
            _delay_ms(1);
        break;
    ...
    uint8_t msg, uint8_t arg_int, void* arg_ptr)
{
    ...
    switch (msg) {
    case U8X8_MSG_GPIO_AND_DELAY_INIT: // called
                                       // once
                                       // during
                                       // init
                                       // phase of
    ...
    case U8X8_MSG_GPIO_SPI_CLOCK: // Clock pin:
                                  // Output level
                                  // in arg_int
        if (arg_int)
            DISPLAY_CLK_PORT
            ...
            DISPLAY_CLK_PORT
                &= ~(1 << DISPLAY_CLK_PIN);
        break;
    case U8X8_MSG_GPIO_SPI_DATA: // MOSI pin:
                                 // Output level
                                 // in arg_int
        if (arg_int)
            DISPLAY_DATA_PORT
            ...
            DISPLAY_DATA_PORT
                &= ~(1 << DISPLAY_DATA_PIN);
        break;
    case U8X8_MSG_GPIO_CS: // CS (chip select)
                           // pin: Output level in
                           // arg_int
        if (arg_int)
            DISPLAY_CS_PORT
            ...
            DISPLAY_CS_PORT
                &= ~(1 << DISPLAY_CS_PIN);
        break;
    case U8X8_MSG_GPIO_DC: // DC (data/cmd, A0,
                           // register select)
                           // pin: Output level in
                           // arg_int
        if (arg_int)
            ...
            DISPLAY_DC_PORT
                &= ~(1 << DISPLAY_DC_PIN);
        break;
    case U8X8_MSG_GPIO_RESET: // Reset pin: Output
                              // level in arg_int
        if (arg_int)
            DISPLAY_RESET_PORT
                |= (1 << DISPLAY_RESET_PIN);
        ...
        break;
    default:
        if (u8x8_avr_delay(u8x8,
                U8X8_MSG_DELAY_NANO, arg_int,
                arg_ptr)) // check for any delay
                          // msgs
            return 1;
        u8x8_SetGPIOResult(
            u8x8, 1); // default return value
        break;
    }
    return 1;
}
...
void eepromUpdate(
    unsigned int address, float data)
{
    /* Wait for completion of previous write */
    while (EECR & (1 << EEPE)) { }
...
void setup_timer2(void)
{
    ...
    // Set for ~5ms interval at 8MHz
    // (8,000,000 / 1024) for 1 sec at 8M with
    // 1024 prescaler (8,000,000 / 1024) / 200 =
    // 39
    OCR2A = 39;
    // 1024 prescaler
    TCCR2B
...
ISR(TIMER2_COMPA_vect)
{
    timer2Flag = 1;
    timer2Overflows++;
    timer2LCDRefreshCounter++;
    if (timer2LCDRefreshCounter == 200) {
        LcdTick = 1; // Now the LCD refreshes from
                     // here!
...
void lcd_init(void)
{
   ...
   */
    ...
    u8g2_SetFont(
        &u8g2, u8g2_font_prospero_nbp_tr);
...
{
    clock_prescale_set(
        clock_div_1); // set the clock prescaler
                      // to 1

    // Set input ports for buttons
    DDRB &= ~((1 << 1) | (1 << 4) | (1 << 6)
        | (1 << 7)); // 1 and 4 are the Enter and
                     // Back buttons
    DDRD &= ~((1 << 2) | (1 << 6)
        | (1 << 7)); // 2 is for checking pc
                     // connection
    ...
    TCCR0A |= (1 << WGM01) | (1 << WGM00)
        | (1 << COM0B1); // set fast PWM Mode and
                         // non-inverting mode
    ...
    if (((int)eepromRead(MEASUREMENT_1_ADDRESS)
            >= 1)
        && ((int)eepromRead(MEASUREMENT_1_ADDRESS)
            <= 15)) {
        for (int i = 0; i < 6; i++) {
            if (i < 4) {
                MenuSettings[i]
                    = (eepromRead(
                           MenuSettingsEEPROMAddresses
                               [i])
                        - 1); // Read values from
                              // addresses if
                              // values are
        ...
        for (int i = 0; i < 6;
            i++) { // Write initial values to menu
                   // setting addresses
            if (i == 4) {
                eepromUpdate(i,
                    (float)4); // Initial
    ...
    OCR0B = Backlight
        [BacklightIndexCopy]; // Set Backlight
...
{
    u8g2_SetFont(&u8g2,
        u8g2_font_u8glib_4_hr); // Change to
                                // smaller font
    if (!Menu) {
        u8g2_SetDrawColor(&u8g2, 2);
    } else {
        u8g2_SetDrawColor(&u8g2, 0);
        u8g2_DrawBox(&u8g2, 0, 51, 128, 13);
...
{
    for (int i = 0; i < 5; i++) {
        if (i == SelectPosition[0]) { // Selection
                                      // Box
            u8g2_SetDrawColor(&u8g2, 1);
            u8g2_DrawBox(
                &u8g2, 0, (i * 13) - 1, 128, 13);
            u8g2_SetDrawColor(&u8g2, 0);
        } else {
            u8g2_SetDrawColor(&u8g2, 0);
            u8g2_DrawBox(
        ...
        if (i < 4) {
            u8g2_DrawStr(&u8g2, 1, 10 + (i * 13),
                MeasurementName[(int)MenuSettings
                        [i]]); // MenuSettings
                               // stores the index
                               // for which
                               // measurement it
                               // is
            if (measurements[(int)MenuSettings[i]]
                == 37) {
                u8g2_DrawStr(&u8g2, 47,
                    10 + (i * 13), "UL");
            } else {
                ...
                int measurementInt = measurements
                    [(int)MenuSettings[i]
                        + arrayOffset];
                int afterDecimal
                    = ((measurements
                    ...
                    * 100;
                if (afterDecimal < 10) {
                    sprintf(measurementBuffer,
                        "%d.0%d",
                        (int)measurementInt,
                        (int)round(
                            afterDecimal)); // Convert
                                            // Measurement
                                            // float
                                            // value
                                            // to
    ...
    u8g2_SetDrawColor(&u8g2, 2);
    u8g2_DrawLine(&u8g2, 44, 1, 44,
        48); // Line separating measurement name
...
{
    MainScreenDraw(
        measurements); // Refreshes display
                       // constantly to display
                       // changing values.
    if (ButtonTurn) { // Attempts to prevent
                      // pressing of multiple
                      // buttons.
        if (ENTER || RIGHT) { // Enter and right
            ...
            if (SelectPosition[0] < 4) {
                Menu = 1; // Measurement select
                          // menu
                SelectPosition[1] = (int)
                    MenuSettings[SelectPosition
                            [0]]; // Set position
                                  // at the index
                                  // of the
                                  // measurement
...
void MeasurementSelectDraw()
{
    for (int i = 0; i < 4; i++) {
        ...
        int idx;
        if (i
            < SelectPosition[0]) { // Shows
                                   // measurements
                                   // above select
            idx = SelectPosition[1]
                - (SelectPosition[0] - i);
        } else { // Shows measurements below
                 // select
            idx = SelectPosition[1]
                + (i - SelectPosition[0]);
        }
...
{
    u8g2_SetFont(&u8g2,
        u8g2_font_prospero_nbp_tr); // Reset font
                                    // after
                                    // returning
                                    // from
                                    // TurnsRatioMenuDraw
    for (int i = 0; i < 2; i++) {
        ...
        if (i == 0) {
            u8g2_DrawStr(
                &u8g2, 1, 10, "Backlight:");
            u8g2_DrawTriangle(&u8g2, 60, 1, 53, 5,
                60, 9); // Arrows
            u8g2_DrawTriangle(
                &u8g2, 70, 1, 77, 5, 70, 9);
            char Index[2];
            sprintf(Index, "%d",
                BacklightIndexCopy); // Convert
                                     // int
                                     // BacklightIndexCopy
                                     // to string
            ...
            u8g2_DrawStr(
                &u8g2, 1, 25, "Turns Ratio:");

            // Convert TurnsRatio float value to
            // string
            char turnsRatioBuffer[20];
            int turnsRatioInt = TurnsRatioCopy;
            int afterDecimal
                = (TurnsRatioCopy - turnsRatioInt)
                * 1000;
            if (afterDecimal < 10) {
                sprintf(turnsRatioBuffer,
                    "%d.00%d", (int)turnsRatioInt,
                    (int)round(afterDecimal));
            } else if ((afterDecimal < 100)
                && (afterDecimal >= 10)) {
                sprintf(turnsRatioBuffer,
                    "%d.0%d", (int)turnsRatioInt,
                    (int)round(afterDecimal));
            } else {
                sprintf(turnsRatioBuffer, "%d.%d",
                    (int)turnsRatioInt,
                    (int)round(afterDecimal));
            }

            u8g2_DrawStr(
                &u8g2, 62, 25, turnsRatioBuffer);
        }
    }
    PCIconAndTime();
}
    ...
    if (ButtonTurn) {
        if (ENTER) { // Enter
            ...
            if (SelectPosition[1]
                ...
                OCR0B = Backlight[(
                    int)MenuSettings[4]];
                eepromUpdate(BACKLIGHT_ADDRESS,
                    BacklightIndexCopy);
                u8g2_ClearBuffer(&u8g2);
            ...
            ButtonTurn = 1;
        } else if (BACK) { // Back. Exit settings
                           // without saving
                           // backlight setting
                           // changes
            ButtonTurn = 0;
            ...
            u8g2_DrawBox(&u8g2, 0, 0, 128, 64);
            Menu = 0;
            BacklightIndexCopy = (int)MenuSettings
...
void TurnsRatioMenuDraw()
{
    ...
    u8g2_SetDrawColor(&u8g2, 0);
    u8g2_DrawStr(&u8g2, 1, 10, "Turns Ratio:");
    char turnsRatioBuffer[20];
    int turnsRatioInt = MenuSettings[5];
    int afterDecimal
        = (MenuSettings[5] - turnsRatioInt)
        * 1000;
    if (afterDecimal < 10) {
        sprintf(turnsRatioBuffer, "%d.00%d",
            (int)turnsRatioInt,
            (int)round(
                afterDecimal)); // Convert
                                // Measurement
        ...
        && (afterDecimal >= 10)) {
        sprintf(turnsRatioBuffer, "%d.0%d",
            (int)turnsRatioInt,
            (int)round(
                afterDecimal)); // Convert
                                // Measurement
    ...
    u8g2_DrawStr(&u8g2, 62, 10, turnsRatioBuffer);

    int DecimalOffset
        = 0; // Used to offset the x position due
             // to the decimal
    int digitSpace = 16;
    for (int i = 0; i < 5; i++) {
        u8g2_SetDrawColor(&u8g2, 1);
        ...
        int Offset = 21 + DecimalOffset;
        u8g2_DrawTriangle(&u8g2,
            (i * digitSpace) + Offset, 21,
            (i * digitSpace) + 3 + Offset, 15,
            (i * digitSpace) + 7 + Offset,
            21); // Arrows
        u8g2_DrawTriangle(&u8g2,
            (i * digitSpace) + Offset, 46,
            (i * digitSpace) + 3 + Offset, 51,
            (i * digitSpace) + 7 + Offset, 46);
        if (i != SelectPosition[1]) {
            u8g2_SetDrawColor(&u8g2, 0);
            u8g2_DrawBox(&u8g2,
                (2 * digitSpace) + 17, 23, 16,
                21);
        }
        u8g2_DrawBox(&u8g2,
            (i * digitSpace) + Offset - 4, 23, 16,
            21);
    }
    ...
    char turnsRatioBuffer2[20];
    turnsRatioInt = TurnsRatioCopy;
    afterDecimal
        = (TurnsRatioCopy - turnsRatioInt) * 1000;
    if (afterDecimal < 10) {
        sprintf(turnsRatioBuffer2, "%d.00%d",
            (int)turnsRatioInt,
            (int)round(
                afterDecimal)); // Convert
                                // Measurement
        ...
        && (afterDecimal >= 10)) {
        sprintf(turnsRatioBuffer2, "%d.0%d",
            (int)turnsRatioInt,
            (int)round(
                afterDecimal)); // Convert
                                // Measurement
...
void TurnsRatioMenu()
{
    TurnsRatioMenuDraw();
    if (ButtonTurn) {
        if (ENTER) { // Enter. Confirm New Turn
                     // Ratio
            ButtonTurn = 0;
            u8g2_SetDrawColor(&u8g2, 0);
            u8g2_DrawBox(&u8g2, 0, 0, 128, 64);
            ...
            turnsRatio = MenuSettings[5];
            eepromUpdate(TURNS_RATIO_ADDRESS,
                (float)turnsRatio);
            Menu = 2;
            ButtonTurn = 1;
            u8g2_ClearBuffer(&u8g2);
            ...
            u8g2_SendBuffer(&u8g2);
            ButtonTurn = 1;
        } else if (BACK) { // Back. Return without
                           // saving Turns Ratio
                           // changes
            ButtonTurn = 0;
            u8g2_SetDrawColor(&u8g2, 0);
            ...
            ButtonTurn = 0;
            int digit = (TurnsRatioCopy
                / pow(
                    10, 1 - (SelectPosition[1])));
            digit = digit % 10;
            if (digit
                == 9) { // Check specific digit
                TurnsRatioCopy -= (90
                    / pow(10,
                        SelectPosition
                            [1])); // Up from 9,
...
int main(void)
{
    ...
    while (1) {
        // Process new measurement data if the
        // window is ready
        ...
        if (!Menu) { // The different menu
                     // screens.
            MainScreen(measurements);
        } else if (Menu == 1) {
            MeasurementSelect(measurements);
        } else if (Menu == 2) {
